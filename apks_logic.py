...
async def start_upload_process(message: Message, state: FSMContext):
    ...
    await state.set_state(UploadHTML.html_required)

async def handle_html_document(message: Message, state: FSMContext):
    await receive_html_file(message, state)  # Викликаємо існуючу функцію обробки

async def invalid_input_in_state(message: Message):
...
async def receive_html_file(message: Message, state: FSMContext):
    ...
    try:
        ...
        xlsx_file = BufferedInputFile(
            xlsx_io.read(),
            filename=f"APKS_{safe_username}.xlsx"
        )
        ...
        caption = (
            ...
            f"Готовий XLSX: <b>APKS_{safe_username}.xlsx</b>\n"
        ...
        try:
            ...
            await conn.executemany(f'''
                INSERT INTO "APKS" (question, answer, added_by)
                VALUES ($1, $2, $3)
